...
if [[ "$(curl -SsL https://api.github.com/repos/gsoc2/${BIN}/releases/tags/${VERSION} | jq .prerelease)" == "true" ]] ; then
...
aws s3 cp "${VERSION_FILE}" s3://toolbox-data.gsoc2.io/${BIN}/releases/latest/VERSION
