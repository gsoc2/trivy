...
GOIMPORTS_CMD := $(TEMP_DIR)/gosimports -local github.com/gsoc2
...
static-analysis: check-go-mod-tidy lint check-licenses validate-trivy-db-schema

.PHONY: test
test: unit integration validate-cyclonedx-schema validate-trivy-db-schema cli ## Run all tests (unit, integration, linux acceptance, and CLI tests)

.PHONY: validate-cyclonedx-schema
validate-cyclonedx-schema:
	cd schema/cyclonedx && make

.PHONY: validate-trivy-db-schema
validate-trivy-db-schema:
	# ensure the codebase is only referencing a single trivy-db schema version, multiple is not allowed
	python test/validate-trivy-db-schema.py
...
unit: $(TEMP_DIR) ## Run unit tests (with coverage)
	...
	go test -race -coverprofile $(TEMP_DIR)/unit-coverage-details.txt $(shell go list ./... | grep -v gsoc2/trivy/test)
...
integration:  ## Run integration tests
	...
	go run cmd/trivy/main.go alpine:latest
...
.PHONY: validate-trivy-test-config
validate-trivy-test-config:
	# ensure the update URL is not overridden (not pointing to staging)
	@bash -c '\
		grep -q "update-url" test/trivy-test-config.yaml; \
		if [ $$? -eq 0 ]; then \
